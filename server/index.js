...
module.exports = function(config) {
  ...
  function docRequest(req, res, next) {
    ...
    docFilepath(req, function(nope, filepath) {
      ...
      function done() {
        ...
        res.render(content.context.template || 'default');
      }

      if (!content.context.aggregateChildren) {
      ...
      debug('populate sub pages for %s', dirname);
      utils.glob(dirname +'/**/*.{md,html}', function(err, files) {
        if (err) {
          return next(err);
        }


        debug('analyse %d pages', files.length);
        var subPages = {};
        _.each(files, function(file) {
          var subContent = yfm.read(file);
          if (subContent) {
